...
def fit_multitarget_model(model, X_train, Y_train, X_test, Y_test, actuals_and_forecast_df, targets):
    try:
        ...
        SPCI_class_1.fit_bootstrap_models_online_multistep(B=1, fit_sigmaX=False, stride=stride)
        use_SPCI = False
        smallT = not use_SPCI
        ...
        SPCI_class_2.fit_bootstrap_models_online_multistep(B=1, fit_sigmaX=False, stride=stride)
        use_SPCI = False
        smallT = not use_SPCI
...
rolling_walk_forward_validation(model=RandomForestRegressor(max_depth=70, max_features=150, n_estimators=300),
                                data=dat, start_time='1/6/2020 00:00', end_time='1/9/2020  00:00',
                                targets=Y.columns.values.tolist(), training_days=-210,
                                path="/home/coconnor/rf_EnbPI_DAM_1-3")